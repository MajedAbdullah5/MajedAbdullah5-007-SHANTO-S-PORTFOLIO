...
$('#addProfilePhoto').click(function () {
    $('#addProfilePictureModal').modal('show');
});
$('#addProfilePictureModalButton').click(function () {
    let file = $('#addProfilePictureID').prop('files')[0];
    let formData = new FormData();
    formData.append('file', file);
    axios.post('/addProfilepicture', formData).then(function (response) {
        if (response.status == 200) {
...
$('#addObjectiveButton').click(function () {
    $('#addObjectives').modal('show');
});

$('#addObjectiveModalButton').click(function () {
    let objectivesId = $('#addObjectivesID').val();
    axios.post('/addObjectives', {
